...
network_fn = nets_factory.get_network('cmu_paf_fpn_net', is_training=False)
...
ckpt_filename = '/home/xiaoyu/Documents/AI_keypoint/log*/model.ckpt-30209'
isess.run(tf.global_variables_initializer())
...
for item in s:
    ...
    heatmaps_avg = show_gaussian#heatmaps[0] + heatmaps[1] + heatmaps[2] + heatmaps[3]; 
    pafs_avg = show_vec#pafs[0] + pafs[1] + pafs[2] + pafs[3]; 
...
# #     temp = {}
# #     temp['image_id']= os.path.split(imgpath)[1][:-4]
...
# # with open('./submit_0.json', 'w') as json_file:
# #     json_file.write(json.dumps(result))